...
def process_db_parallel(base_dir, th=0.11):
    ...
    """
    db_path = osp.join(base_dir, 'mcg/ucm.mat')
    out_path = osp.join(base_dir, '../data/datasets/seg_uint16.h5')
    print(os.path.exists(db_path), os.path.exists(out_path))
    # output h5 file:
    ...
    class ucm_iterable(object):
        def __init__(self, ucm_path, th):
            self.th = th
            self.ucm_h5 = h5py.File(ucm_path, 'r')
...
if __name__ == '__main__':
    base_dir = ''  # directory containing the ucm.mat, i.e., output of run_ucm.m
