...
def test(dataset_name, model_name, store_misclassified):
    ...
    with tf.Session() as sess:
        ...
        confusion = (confusion / np.array([np.sum(confusion, 1)]).transpose() * 255).astype(np.uint8)
        cv2.imshow('Confusion', cv2.resize(confusion, None, fx=10, fy=10, interpolation=cv2.INTER_NEAREST))
        cv2.waitKey(-1)
