...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        nm, nf, ne, ns, nd, nc = 0, 0, 0, 0, 0, 0  # number missing, found, empty, datasubset, duplicate
        ...
        for i, file in enumerate(pbar):
            try:
                ...
                nc += 1
                print(f'WARNING: Ignoring corrupted label {file}: {e}')

            pbar.desc = 'Scanning labels %s (%g found, %g missing, %g empty, %g duplicate, %g corrupted, for %g images)'\
                        % (cache_path, nf, nm, ne, nd, nc, n)
        if nf == 0:
