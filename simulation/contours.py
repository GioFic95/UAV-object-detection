...
def crop_image(in_path, out_path, name):
    ...
    for i, c in enumerate(contours):
        ...
        x0 = x - w0 - r_margin if r_margin > 0 else x - w0 if l_margin <= 0 else x - w0 + l_margin
        x1 = x + w + w0 + l_margin if l_margin > 0 else x + w + w0 if r_margin <= 0 else x + w + w0 - r_margin
        y0 = y - h0 - b_margin if b_margin > 0 else y - h0 if t_margin <= 0 else y - h0 + t_margin
        y1 = y + h + h0 + t_margin if t_margin > 0 else y + h + h0 if b_margin <= 0 else y + h + h0 - b_margin
